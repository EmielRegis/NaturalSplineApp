...
type interval = record
                  a, b : Extended;
                  // Overloaded class operators - by Karol
                  class operator Add(const x, y: interval) : interval;
                  class operator Subtract(const x, y : interval) : interval;
                  class operator Multiply(const x, y : interval) : interval;
                  class operator Divide(const x, y : interval) : interval;
                  class operator Negative(const x: interval) : interval;
...
// New added functions - by Karol
function iabs(const x : interval) : interval;
function iln(const x : interval) : interval;
function isinh(const x : interval) : interval;
function iarctg(const x : interval) : interval;
function iarcsinh(const x : interval) : interval;
function iarcctg(const x : interval) : interval;
function iarctgh(const x : interval) : interval;
...
// New pseudo constructors - added by Karol
function inew(const x, y : extended) : interval; overload;
function inew(const x : string) : interval; overload;
function inew(const x,y : string) : interval; overload;
...
{2^(-60)}   '0.000000000000000000867361737988403547205962240695953369140625000',
  ...
  class operator interval.Add(const x, y: interval): interval;
  begin
    SetRoundMode (rmDown);
    Result.a:=x.a+y.a;
    SetRoundMode (rmUp);
  ...
  class operator interval.Subtract(const x, y : interval) : interval;
  begin
    SetRoundMode (rmDown);
    Result.a:=x.a-y.b;
    SetRoundMode (rmUp);
  ...
  class operator interval.Multiply(const x, y : interval) : interval;
  ...
  class operator interval.Divide(const x, y : interval) : interval;
  var x1y1, x1y2, x2y1 : Extended;
  begin
    if (y.a<=0) and (y.b>=0)
  ...
  class operator interval.Negative(const x: interval) : interval;
  begin
    Result.a := -x.b;
    Result.b := -x.a;
  end;
  ...
  begin
     if ((x.a > y.b) and (x.b > y.b) and (x.a > y.a) and (x.b > y.a)) then
     begin
      Result := true;
     end
     else
  ...
  begin
     if ((x.a > y.b) and (x.b > y.b) and (x.a > y.a) and (x.b > y.a)) or ((x.a = y.a) and (x.b = y.b)) then
     begin
      Result := true;
     end
     else
  ...
  begin
    ...
    begin
      ...
      begin
        minAbs := min(abs(x.a), abs(x.a));
      end;

      maxAbs := max(abs(x.a), abs(x.b));
    ...
    function iln(const x : interval) : interval;
    begin
      Result.a := min(Ln(x.a), Ln(x.b));
      Result.b := max(Ln(x.a), Ln(x.b));
    end;

    function isinh(const x : interval) : interval;
    begin
      Result.a := min(sinh(x.a), sinh(x.b));
      Result.b := max(sinh(x.a), sinh(x.b));
    end;

    function iarctg(const x : interval) : interval;
    begin
      Result.a := min(ArcTan(x.a), ArcTan(x.b));
      Result.b := max(ArcTan(x.a), ArcTan(x.b));
    end;

    function iarcsinh(const x : interval) : interval;
    begin
      Result.a := min(ArcSinH(x.a), ArcSinH(x.b));
      Result.b := max(ArcSinH(x.a), ArcSinH(x.b));
    end;

    function iarcctg(const x : interval) : interval;
    begin
      Result.a := max(ArcCot(x.a), ArcCot(x.b));
      Result.b := min(ArcCot(x.a), ArcCot(x.b));
    end;
    function iarctgh(const x : interval) : interval;
    begin
      Result.a := max(ArcTanH(x.a), ArcTanH(x.b));
      Result.b := min(ArcTanH(x.a), ArcTanH(x.b));
    end;
...
//    if (Pos('.', sa1)>0) and (DecimalSeparator=',')   { for older versions }
  ...
  function inew(const x, y : extended) : interval;
    begin
      Result.a := x;
      Result.b := y;
    end;

    function inew(const x : string) : interval;
    begin
      Result.a := left_read(x);
      Result.b := right_read(x);
    end;
    function inew(const x,y : string) : interval;
    begin
      Result.a := left_read(x);
      Result.b := right_read(y);
    end;
